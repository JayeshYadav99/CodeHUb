...
const Homepage = () => {
  ...
  const { auth0, isLoading, isAuthenticated, getAccessTokenSilently } =
    useAuth0();
  ...
  const fetchUserInfo = async (ACCESS_TOKEN) => {
    try {
      const response = await axios.get(
        "https://dev-fkzyzzay6f6jrars.us.auth0.com/userinfo",
        {
          headers: {
...
=======
  ...
  async function fetchInst() {
    ...
    try {
      const response = await fetch(
        `https://api.github.com/app/installations/37966220/access_tokens`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      <div className="mt-8">
        <Card className="w-64 bg-gray-900">
          <Card.Img
            variant="top"
            src={avatarURL}
            className="w-32 h-32 object-cover mx-auto mt-6 rounded-full p-2"
          />
          <Card.Body className="p-6">
            <div className="flex flex-col items-center">
              <Card.Title className="font-bold text-xl text-white mb-2">
                Welcome, {githubUsername}!
              </Card.Title>
              <Card.Text className="text-gray-400">
                Thank you for exploring our website. You can get anything from
                everything here.
              </Card.Text>
              <Button
                variant="primary"
            ...
            return (
              <Card
                ...
                className={`flex flex-col justify-between items-center ${
                  repoColors[index % repoColors.length]
                } text-white hover:bg-pink-500 transition duration-300`}
              >
                <Card.Img
                  variant="top"
                  src="github.png"
                ...
                <Card.Body className="p-6 flex flex-col justify-between items-center">
                  <div className="flex flex-col items-center">
                    <Card.Title className="font-bold text-lg mb-2">
                      {repo.name}
                    </Card.Title>
                  ...
                  <div className="flex items-center justify-center w-full mt-4">
                    <Button
                      variant={`primary bg-red-500 hover:bg-white hover:text-black`}
                      href={repo.html_url}
                      target="_blank"
                    ...
                    <Button
                      variant={`secondary bg-black hover:bg-white hover:text-black`}
                      className="mt-2 ml-4 flex items-center"
